...
export class OrderController {
  ...
  @Get('test-mail')
  async testMail() {
    // Attempting to send using an existing user ID (make sure ID 1 exists and has an email, or replace with your test ID)
    await (this.orderService as any).sendOrderConfirmation(1, { 
      id: 999123, 
      totalAmount: 150.50,
      items: [
        { productName: 'Baby Formula', quantity: 2, unitPrice: 75.25 }
      ]
    });
    return { success: true, message: 'Test email triggered! Check logs.' };
  }
