...
export class OrderService {
  ...
  private sendOrderConfirmation(buyerId: number, order: any) {
    console.log(`[OrderService] Attempting to send order confirmation for buyer ${buyerId}, order ${order.id}`);
    this.prisma.user.findUnique({
      where: { id: buyerId },
      select: { email: true, firstName: true }
    }).then(user => {
      console.log(`[OrderService] Found user for order confirmation:`, user?.email);
      if (user && user.email) {
        ...
        console.log(`[OrderService] Calling mailService to send OrderConfirmation to ${user.email}`);
        ...
        .then(() => console.log(`[OrderService] Successfully resolved mailService.sendMail`))
        .catch(err => console.error('[OrderService] Failed to send OrderConfirmation email', err));
      } else {
        console.warn(`[OrderService] User ${buyerId} has no email, skipping order confirmation`);
      }
    }).catch(err => console.error('[OrderService] Error fetching user for order email', err));
  }
